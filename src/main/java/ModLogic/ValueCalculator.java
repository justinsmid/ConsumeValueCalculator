...
public class ValueCalculator implements OnStartBattleSubscriber, PostBattleSubscriber, PostUpdateSubscriber {
    ...
    private void calculate() {
        ...
        int lightningOrbCountPostConsume = (lastOrb instanceof Lightning) ? currentLightningOrbCount - 1 : currentLightningOrbCount;

        int valuePerLightningOrb = (hasLightning) ? getRandomOrb(OrbType.Lightning).passiveAmount : 0;

        Calculation lightningWithConsume = new Calculation(lightningValueWithConsume, lightningOrbCountPostConsume, valuePerLightningOrb + consumeValue, OrbType.Lightning, true);
        calculations.add(lightningWithConsume);
        Calculation lightningWithoutConsume = new Calculation(currentLightningDamage, currentLightningOrbCount, valuePerLightningOrb, OrbType.Lightning, false);
        ...
        int frostOrbCountPostConsume = (lastOrb instanceof Frost) ? currentFrostOrbCount - 1 : currentFrostOrbCount;

        int valuePerFrostOrb = (hasFrost) ? getRandomOrb(OrbType.Frost).passiveAmount : 0;

        Calculation frostWithConsume = new Calculation(frostValueWithConsume, frostOrbCountPostConsume, valuePerFrostOrb + consumeValue, OrbType.Frost, true);
        calculations.add(frostWithConsume);
        Calculation frostWithoutConsume = new Calculation(currentFrostBlock, currentFrostOrbCount, valuePerFrostOrb, OrbType.Frost, false);
